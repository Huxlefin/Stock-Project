...
data = pd.read_csv("updated_sp500_stocksV2.csv")
...
for stock in unique_symbols:
    ...
    plt.plot(range(2000), losses, label="Training Loss")
    plt.xlabel("Epochs")
    plt.ylabel("Loss (MSE)")
    ...
    # Total time for this stock
    stock_end_time = time.time()
    print(f"Total time for {stock}: {stock_end_time - stock_start_time:.2f} seconds")
